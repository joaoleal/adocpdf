...
- [x] 1.1 Add `proptest` 1.11.0 to `[workspace.dependencies]` and as a
...
- [x] 1.2 Survey what `proptest` drags in: run `cargo deny check licenses` and
...
- [x] 2.1 Add Tier 1 property tests for `markup::string_literal` (design D2):
      ...
      not evidence.
- [x] 2.2 Add the Tier 2 property: emit the literal, compile it through the real
      ...
      guess.
- [x] 2.3 Commit the `proptest-regressions/` directory and confirm it is not
...
- [x] 3.1 Add a property test in `adocpdf-domain` generating path expressions
      ...
      *Verified by:* the property passes, and the existing example-based sandbox
      ...
      accommodate it.
- [x] 3.2 Confirm the symlink cases remain integration tests in `adocpdf-infra`
...
- [x] 4.1 Run `cargo fuzz init`, then immediately run the architecture guard and
      ...
      answer.
- [x] 4.2 Record in `deny.toml` and `architecture.toml` that `fuzz/` is outside
...
- [x] 4.3 Add a fuzz target over the pure parse → plan → emit path taking
...
- [x] 4.4 Run the target for a bounded budget and report what it finds, with
...
- [x] 4.5 Convert every crash the run found into a minimised `#[test]` in the
...
- [x] 5.1 Install `cargo-mutants` 27.1.0 and run it against
...
- [x] 5.2 For each survivor in those two files, strengthen the property or add
      ...
      equivalent and cannot be killed, it goes in `.cargo/mutants.toml` with a written
      reason — never silently.
- [x] 5.3 Run `cargo-mutants` across the rest of the workspace once and report
...
- [x] 6.1 Add a scheduled CI workflow running the fuzz targets on nightly with a
...
- [x] 6.2 Add a scheduled CI workflow running `cargo-mutants`, failing on any
...
- [x] 6.3 Document in `AGENTS.md` and `README.md` what each of the three
...
- [x] 6.4 Run `scripts/ci/gate.sh` and confirm it still passes, including the
