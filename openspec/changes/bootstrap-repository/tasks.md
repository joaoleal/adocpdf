...
- [ ] 1.2 Confirm `git status --porcelain` shows no build or coverage artefacts
...
- [ ] 2.1 Commit the `render-first-pdf` work — the six crates under `crates/`,
...
- [ ] 2.2 Commit the `quality-gates` work — `scripts/ci/gate.sh`, `deny.toml`,
      ...
      those paths.
- [ ] 2.3 Commit the OpenSpec planning artifacts (`openspec/`) and the agent
...
- [ ] 3.1 Add `SECURITY.md` stating the four boundaries from design D9 — the
...
- [ ] 3.2 Add `CONTRIBUTING.md` covering the OpenSpec propose → apply → archive
...
- [ ] 3.3 Add `.editorconfig` matching the settings already implied by
...
- [ ] 4.1 Install `actionlint` and `zizmor`, and add both to
...
- [ ] 4.2 Update the job count and the job table in `README.md` and `AGENTS.md`
...
- [ ] 5.1 Add `.github/workflows/ci.yml` running `scripts/ci/gate.sh` on push
      ...
      copied from memory.
- [ ] 5.2 Ensure the workflow installs what the gate needs beyond the tools: the
...
- [ ] 5.3 Install `actionlint` outside `install-action`, which does not carry it
...
- [ ] 5.4 Run `actionlint` and `zizmor` against the new workflow and fix every
...
- [ ] 6.1 Add `.github/dependabot.yml` covering the `cargo` and
      ...
      updated.
- [ ] 6.2 Record in `design.md` that Dependabot's handling of inherited
...
- [ ] 7.1 Run the full `scripts/ci/gate.sh` — now seventeen jobs — and confirm
