...
- [ ] 1.1 Confirm `bootstrap-repository` has been applied: `.github/workflows/`
...
- [ ] 2.1 Record a baseline: wall-clock for a clean `cargo build --release` and
...
- [ ] 2.2 Add `codegen-units = 1` to `[profile.release]`, leaving `lto` at
      ...
      not touched.
- [ ] 2.3 Add `[profile.dev.package."*"] opt-level = 2` and re-measure a clean
      ...
      apply it.
- [ ] 2.4 Confirm the dev-profile change moves neither the coverage figure nor
...
- [ ] 3.1 Install `cargo-auditable` 0.7.5 and build the release binary with it.
      *Verified by:* `cargo audit bin` against the produced binary lists the
      dependency graph.
- [ ] 3.2 Check whether `cargo-auditable` alters the dependency graph, by
...
- [ ] 3.3 Add `strip = "debuginfo"` to `[profile.release]` (design D3), then
...
- [ ] 3.4 Record in `Cargo.toml`, beside the profile, why `panic = "abort"` is
...
- [ ] 4.1 Install `cargo-cyclonedx` 0.5.9 and generate an SBOM for the
      workspace. *Verified by:* the output is valid CycloneDX and lists the
      dependencies that `cargo tree` shows.
- [ ] 4.2 Add SBOM generation to a release CI job, publishing the result as a
...
- [ ] 5.1 Add `committed` 1.1.11 with its configuration, and run it against the
      ...
      accept them.
- [ ] 5.2 Add a CI job running `committed` over a pull request's commit range,
...
- [ ] 5.3 Document in `AGENTS.md` and `CONTRIBUTING.md` that this is the one
...
- [ ] 6.1 Add `cliff.toml` and generate `CHANGELOG.md` from the existing
...
- [ ] 6.2 Document that the changelog is regenerated when a release is cut, not
      per commit (design D6). *Verified by:* the instruction is written down and
      no CI job regenerates it on push.

## 7. Close out

- [ ] 7.1 Run `scripts/ci/gate.sh` and confirm it passes. *Verified by:* the
      gate prints `gate passed`.
- [ ] 7.2 Report which settings survived measurement and which were removed,
